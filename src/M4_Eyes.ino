...
void loop() {
  ...
  // Serial.printf("state: %d eyeNum: %d colNum: %d\n", state, eyeNum, eye[eyeNum].colNum);
  if(state == START_SLEEPING && eye[eyeNum].colNum == 0 && eyeNum+1 == NUM_EYES) {
    Serial.printf("Starting blink.\n"); 
    for(int e = 0; e < NUM_EYES; ++e) {
      timeOfLastBlink = t;
      eye[e].blink.state = ENBLINK;
      eye[e].blink.startTime = t;
      eye[e].blink.duration = 576000;
    }
    timeToNextBlink = 4000000;
    state = SHUTTING_DOWN;
  }
  else if(state == SHUTTING_DOWN && eye[eyeNum].colNum == 0 && eyeNum+1 == NUM_EYES && eye[eyeNum].blink.state > ENBLINK) {
    Serial.printf("Ok switching off.\n"); 
    for(int e = 0; e < NUM_EYES; ++e) {
      eye[e].display->fillScreen(0);
    }
    arcada.setBacklight(0);
